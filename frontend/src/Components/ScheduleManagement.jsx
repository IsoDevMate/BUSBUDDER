...
async function cancelSchedule(id) {
  const token = localStorage.getItem('accessToken');
  const response = await fetch(`${API_URL}/schedules/${id}/cancel`, {
    method: 'PATCH',
    headers: {
      'Authorization': `Bearer ${token}`,
    },
...
function ScheduleManagement() {
  ...
  const fetchRoutes = async () => {
    try {
      ...
      const routeArray = Array.isArray(data.data) ? data.data : [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (editMode && selectedSchedule) {
        response = await updateSchedule(selectedSchedule, schedulePayload);
      } else {
        response = await createSchedule(schedulePayload);
      }

      setServerResponse(response.message || 'Schedule saved successfully');
      fetchSchedules();
    } catch (err) {
      console.error('Error saving schedule:', err);
      setError(editMode ? 'Failed to update schedule' : 'Failed to create schedule');
